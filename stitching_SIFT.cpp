...
int main(int argc, char* argv[])
{
    ...
    int minHessian = 2000;
    cv::Ptr<cv::SIFT> detector1 = cv::SIFT::create(minHessian);
    cv::Ptr<cv::SIFT> detector2 = cv::SIFT::create(minHessian);
	...
	// Detect SURF keypoints and compute descriptors on the CPU
    ...
    cv::imwrite("panoramic_view_SIFT_GPU.jpg", warpedFrame1);
    cv::imwrite("SIFT_GPU_stitching_Matches.jpg", frameMatches);
    ...
    std::cout << "FPS: " << fps <<"\n"<< std::endl;
    cv::imwrite("panoramic_view_SIFT.jpg", warpedFrame1);
    cv::imwrite("SIFT_stitching_Matches.jpg", frameMatches);
